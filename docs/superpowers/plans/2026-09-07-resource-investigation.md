...
- [ ] Navigation: create action_palette.go; :actions snapshots registered visible
...
- [ ] Troubleshooting: create resource_inspector.go; capture selected GVR/path,
...
- [ ] TLS: create internal/inspect/certificate.go; PEM-decode certificate blocks,
...
- [ ] Verification: generated certificates cover valid/expired/not-yet-valid,
  malformed PEM and multiple certificates; UI tests cover literal markup and
  action filtering. Build, focused tests and incremental lint; exercise lab
  command entry points without changing existing networking.
- [ ] Publish a separate branch/PR stacked on native Hubble; keep both unmerged.
